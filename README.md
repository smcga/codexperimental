...
- The `explicitpixels` effect can switch between a generated assignment-only static frame (`mode: "explicit"`) and an audio-reactive procedural animation (`mode: "procedural"`). The current explicit frame is a code-only concert-title-card look (bright center with neon crowd border), and the checked-in assignment frame is treated as source-of-truth; `npm run gen:explicitpixels` preserves it and only refreshes metadata unless you explicitly run `npm run gen:explicitpixels -- --force`.
