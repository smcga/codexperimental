...
- The effect generator modal keeps the workspace cleaner by revealing preview/code panels only when needed, shows a loading showcase of approved community effects while generation is in progress, applies current preview values as submitted defaults automatically, and still includes **Randomize everything** for rapid iteration.
