...
- Transition types include `fade`, `wipe`, `slide-left`, `slide-right`, `slide-up`, `slide-down`, `iris`, `flash`, `shatter`, `signal-collapse`, `camera-punch-through`, `bitplane-wipe` (vertical VGA-style bands with staggered timing), and `packet-loss` (a blocky corrupted-stream handoff with frozen, dropped, and resolving tiles).
