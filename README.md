...
- The `gl_fractal_tunnel` effect uses WebGL2 for a raymarched tunnel/nebula pass; tune `params` like `quality`, `warp`, `hueShift`, `exposure`, and `seed` (and it falls back to `tunnel` if WebGL2 is unavailable).
