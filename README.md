...
| `treegrowth` | `speed`, `levels`, `trunkHeight`, `branchScale`, `branchAngle`, `trunkWidth`, `sway`, `leafSize`, `jitter`, `seed`, `growth` | Cinematic time-lapse tree growth with layered bark, seasonal canopy shifts, and visible age rings; set `growth` to `-1` for auto or `0-1` to override. |
