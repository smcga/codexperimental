...
| `neural_constellation` | `layers`, `neurons`, `pulseSpeed`, `glow`, `hue`, `drift`, `audioReact`, `seed` | Glowing deep-network constellation with inference pulse waves sweeping input to output; audio energy boosts glow and pulse intensity. |
...
- In debug mode (non-`?release=1`), a compact top-right datastore feed now shows short request traces (for example `GET doodles → postgres` or `POST effects → postgres+upstash-kv`) so you can verify which backend path handled each API call while testing. If Postgres is configured but unavailable, the feed surfaces fallback notes (for example `effects fallback: db_client_unavailable`) to make hidden KV fallbacks obvious.
...
- The serverless APIs still support Upstash KV (`KV_*` or `DB2_KV_*`) for counters, rate limits, and generate telemetry, but moderation-backed doodle/effect records now prefer Prisma Postgres when a database URL is configured (`prisma_postgres_PRISMA_DATABASE_URL` first, then `prisma_postgres_POSTGRES_URL`, then `PRISMA_DATABASE_URL`, then `POSTGRES_URL`).
...
- Prisma/Postgres setup (recommended for production moderation data):
  - Set one of these env vars (first non-empty wins): `prisma_postgres_PRISMA_DATABASE_URL`, `prisma_postgres_POSTGRES_URL`, `PRISMA_DATABASE_URL`, or `POSTGRES_URL`.
  - Run `npm run prisma:migrate:dev` for local schema migrations.
...
- In editor mode, **Revert to File** and **Discard local draft** are intentionally different: **Revert to File** resets the currently loaded timeline back to `timeline.release.json` *and* clears the saved local draft, while **Discard local draft** only deletes the saved local draft copy and keeps whatever timeline is currently loaded in the editor.
...
- Scene Actions now include **Split**: clicking **Split** always targets whichever scene is currently active under the playhead timestamp (even if a different scene is selected), and splits it into two back-to-back copies (first half keeps original start, second half starts at the split time, and both preserve original params/layers/automation). If no scene is active at the playhead, Split falls back to the selected scene.
