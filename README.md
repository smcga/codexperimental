...
- Sections can optionally define `overlays.blendIllusions` to enable canvas blend-mode illusions (`mode`: `torch`, `glow`, `moire`, or `all`, plus `intensity` to scale the effect).
- The timeline includes 3D showcase effects: `proper3d` (perspective projection + lighting), `sphere3d` (point-cloud sphere), and `fake3d` (2D skew/shading tricks).
