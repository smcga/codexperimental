...
- The `roadDrive` effect renders a WebGL2 night highway drive with dashed lane markers, guardrail glow, and horizon fog; tune `params` like `speed`, `roadWidth`, `laneDashLength`, `laneGap`, `fog`, `glow`, `cameraBob`, `bassReactive`, and `rmsReactive`. It falls back to the `isogrid` effect when WebGL2 is unavailable.
...
| `textmode_charset` | `cols`, `rows`, `glyphSet`, `mode`, `speed`, `palette`, `scanlines`, `seed` | Coarse ASCII/textmode renderer with palette and glyph ramp presets. |
| `tunnel` | `speed` |  |
| `rotozoom` | `speed` |  |
...
| `roadDrive` | `speed`, `roadWidth`, `laneDashLength`, `laneGap`, `fog`, `glow`, `cameraBob`, `bassReactive`, `rmsReactive` | Falls back to `isogrid` when WebGL2 is unavailable. |
