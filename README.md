...
- The `gl_crystal_cathedral` effect renders a WebGL2 raymarched crystalline nave with reflective floor, repeating arches, and controlled audio-reactive breathing; tune `params` like `speed`, `quality`, `exposure`, `hueShift`, `crystalDensity`, `symmetry`, `reflectivity`, `fog`, `glow`, `audioReact`, `beatKick`, `seed`, and `facetSharpness`. It falls back to a stylized Canvas2D cathedral pass when WebGL2 is unavailable.
...
| `gl_crystal_cathedral` | `speed`, `quality`, `exposure`, `hueShift`, `crystalDensity`, `symmetry`, `reflectivity`, `fog`, `glow`, `audioReact`, `beatKick`, `seed`, `facetSharpness` | Raymarched crystalline cathedral with reflective floor and repeating arches; falls back to a stylized Canvas2D cathedral pass when WebGL2 is unavailable. |
