...
- Append `?editor=1` in dev builds to open the Scene + Timeline Editor (or toggle "Editor mode" in the debug overlay). The editor shows a live preview, includes a "Portrait mobile preview" toggle for phone framing, edits hot-apply to the running demo, and changes persist to localStorage.
