...
| `platformerScroll` | `speed`, `seed`, `tileSize`, `groundRatio`, `parallaxFar`, `parallaxMid`, `parallaxFront`, `audioReact`, `beatKick`, `platformRate`, `platformMaxSteps`, `skyGlow`, `speedLines`, `collectibleRate` | Deterministic side-scrolling platformer parallax scene with twinkling sky glow, speed streaks, collectibles, and a colorful neon astronaut mascot runner. |
