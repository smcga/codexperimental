...
- The `roadDrive` effect renders a WebGL2 night highway drive with dashed lane markers, guardrail glow, and horizon fog; tune `params` like `speed`, `roadWidth`, `laneDashLength`, `laneGap`, `fog`, `glow`, `cameraBob`, `curveStrength`, `curveFrequency`, `curveSpeed`, `bassReactive`, and `rmsReactive`. It falls back to the `isogrid` effect when WebGL2 is unavailable.
...
| `roadDrive` | `speed`, `roadWidth`, `laneDashLength`, `laneGap`, `fog`, `glow`, `cameraBob`, `curveStrength`, `curveFrequency`, `curveSpeed`, `bassReactive`, `rmsReactive` | Falls back to `isogrid` when WebGL2 is unavailable. |
