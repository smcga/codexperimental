...
- The `waterDrops` effect renders a camera-glass condensation overlay with static droplets, slow drips, rim shading, and highlights; tune `params` like `seed`, `density`, `size`, `speed`, `dripRate`, `opacity`, `highlight`, `rim`, `streaks`, `distort`, and `distortScale`.
...
| `waterDrops` | `seed`, `density`, `size`, `speed`, `dripRate`, `opacity`, `highlight`, `rim`, `streaks`, `distort`, `distortScale` | Screen-space wet-glass droplets with optional low-cost distortion sampling throttled every 3 frames. |
