...
- The `gl_crystal_cathedral` effect renders a WebGL2 crystalline cathedral flythrough with reflective flooring, repeating ceremonial arches, and musical breathing accents; tune `params` like `speed`, `quality`, `exposure`, `hueShift`, `crystalDensity`, `symmetry`, `reflectivity`, `fog`, `glow`, `audioReact`, `beatKick`, `seed`, and `facetSharpness`. It falls back to a stylised Canvas2D cathedral pass when WebGL2 is unavailable.
...
| `gl_crystal_cathedral` | `speed`, `quality`, `exposure`, `hueShift`, `crystalDensity`, `symmetry`, `reflectivity`, `fog`, `glow`, `audioReact`, `beatKick`, `seed`, `facetSharpness` | Raymarched crystalline cathedral; falls back to stylised Canvas2D cathedral rendering when WebGL2 is unavailable. |
