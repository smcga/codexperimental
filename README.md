...
- The `treegrowth` effect renders a cinematic time-lapse tree with layered bark highlights, seasonal canopy changes, and visible age-ring markers that communicate passing years; tune `params` like `speed`, `levels`, `trunkHeight`, `branchScale`, `branchAngle`, `trunkWidth`, `sway`, `leafSize`, `jitter`, `seed`; growth always proceeds automatically from simulation time.
...
| `treegrowth` | `speed`, `levels`, `trunkHeight`, `branchScale`, `branchAngle`, `trunkWidth`, `sway`, `leafSize`, `jitter`, `seed` | Cinematic time-lapse tree growth with layered bark, seasonal canopy shifts, and visible age rings; growth proceeds automatically over time. |
