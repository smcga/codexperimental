...
- The `explicitpixels` effect can switch between a generated assignment-only static frame (`mode: "explicit"`) and an audio-reactive procedural animation (`mode: "procedural"`). The checked-in assignment frame is treated as source-of-truth for code-only workflows; `npm run gen:explicitpixels` now preserves it and only refreshes metadata unless you explicitly run `npm run gen:explicitpixels -- --force`.
