...
- The effect generator modal now keeps the initial prompt editor focused and roomy, then reveals preview/code/param sections only after a successful generation. While generating, it shows approved community effects in a carousel with previous/next controls plus live param controls for the currently previewed approved effect, and the eventual submission uses the current preview param values as defaults automatically (no separate “set defaults” step).
