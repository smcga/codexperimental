...
- Scene Actions now include **Split**: with a scene selected, clicking **Split** cuts that scene at the current playhead timestamp into two back-to-back copies of the same scene/effect (first half keeps original start, second half starts at the split time, and both preserve original params/layers/automation).
