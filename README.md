...
- The `waterDropsOverlay` effect renders refractive lens droplets and subtle trails over existing visuals without clearing the frame; tune `params` like `density`, `maxDrops`, `speed`, `distortion`, and `highlight`.
...
| `waterDropsOverlay` | `density`, `maxDrops`, `speed`, `distortion`, `highlight` | Transparent screen-space droplet refraction overlay with glossy highlights and optional streaking trails. |
