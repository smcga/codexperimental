...
| `chromaticAberration` | `strength`, `falloff`, `angle`, `centerX`, `centerY`, `sampleStep`, `alpha`, `animate`, `edgeBoost`, `mode` | Screen-space RGB channel separation that grows from center to edges; `mode` supports `rgb`, `rg`, `rb`, `gb`. |
