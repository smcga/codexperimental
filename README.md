...
- The `roadDrive` effect renders a WebGL2 night highway drive with curved corners, roadside lamps, dashed lane markers, guardrail glow, and horizon fog; tune `params` like `speed`, `roadWidth`, `laneDashLength`, `laneGap`, `fog`, `glow`, `cameraBob`, `bassReactive`, `rmsReactive`, `curveAmount`, `curveSpeed`, `curveWobble`, `lampSpacing`, `lampIntensity`, and `lampReactive`. It falls back to the `isogrid` effect when WebGL2 is unavailable.
...
| `roadDrive` | `speed`, `roadWidth`, `laneDashLength`, `laneGap`, `fog`, `glow`, `cameraBob`, `bassReactive`, `rmsReactive`, `curveAmount`, `curveSpeed`, `curveWobble`, `lampSpacing`, `lampIntensity`, `lampReactive` | Falls back to `isogrid` when WebGL2 is unavailable. |
