...
- Append `?editor=1` in dev builds to open the Scene + Timeline Editor (or toggle "Editor mode" in the debug overlay). The editor shows a live preview, includes a viewport switcher for desktop/mobile sizing, edits hot-apply to the running demo, and changes persist to localStorage.
