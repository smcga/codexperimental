...
| `platformerScroll` | `speed`, `seed`, `tileSize`, `groundRatio`, `parallaxFar`, `parallaxMid`, `parallaxFront`, `audioReact`, `beatKick`, `platformRate`, `platformMaxSteps` | Deterministic side-scrolling platformer parallax scene with looping platforms and a colorful neon astronaut mascot runner. |
...
- Generated runtime code is sandboxed by policy checks: dynamic code execution primitives (for example `Function`, `new Function`, `eval`, `import()`, or string-based `setTimeout`/`setInterval`) are explicitly disallowed and rejected before preview/submission.
- For best generation reliability, favor prompts that imply deterministic helper functions, lookup tables, and explicit state updates rather than runtime code generation tricks.
...
- The timeline editor now includes an FLStudio-style multi-track playlist lane: clips are arranged across tracks, you can wheel-zoom around the cursor, click-drag empty lane space to pan, and drag clips to retime/rearrange sections quickly.
...
- The effect generator modal now keeps the initial prompt editor focused and roomy, then reveals preview/code/param sections only after a successful generation. While generating, it shows approved community effects in a carousel with previous/next controls (kept available whenever the busy panel is open), includes a corner cog button to quickly show/hide community param controls, and uses a dedicated busy panel beneath the preview so controls do not overlap the prompt/canvas; the eventual submission uses the current preview param values as defaults automatically (no separate “set defaults” step).
