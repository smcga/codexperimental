...
- Scene Actions now include **Split**: clicking **Split** cuts the selected scene at the current playhead timestamp, or (if nothing is selected) automatically splits whichever scene is currently active under the playhead into two back-to-back copies (first half keeps original start, second half starts at the split time, and both preserve original params/layers/automation).
