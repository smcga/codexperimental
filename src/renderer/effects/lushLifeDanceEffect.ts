...
type HairProfile = { widthMul: number; dropMul: number; waveMul: number };
...
export class LushLifeDanceEffect implements Effect {
  render({ ctx, width, height, time, audio, params }: EffectRenderContext): void {
    ...
    const drawHair = (): void => {
      ...
      const hairProfile = resolveLushLifeHairProfile(cfg.fashionDetail);
      const hairW = headRadius * hairProfile.widthMul;
      const hairDrop = headRadius * hairProfile.dropMul;
      const wave = hairProfile.waveMul;
      ctx.save();
      ...
      ctx.beginPath();
      ctx.moveTo(joints.head[0] - hairW * 0.08, joints.head[1] - headRadius * 0.12);
      ctx.bezierCurveTo(
        joints.head[0] + hairW * 0.24,
        joints.head[1] - headRadius * 0.18,
        joints.head[0] + hairW * 0.34 + sway * 0.12,
        joints.head[1] + hairDrop * 0.24,
        joints.head[0] + hairW * 0.1,
        joints.head[1] + hairDrop * 0.45
      );
      ctx.fill();

      ctx.strokeStyle = `hsla(${(cfg.hairHue - 8).toFixed(1)} 72% 44% / 0.35)`;
      ctx.lineWidth = Math.max(1.2, dancerScale * 0.006);
      ctx.beginPath();
      ctx.moveTo(joints.head[0] - hairW * 0.22, joints.head[1] - headRadius * 0.04);
      ctx.quadraticCurveTo(joints.head[0] - hairW * 0.05, joints.head[1] + hairDrop * 0.32, joints.head[0] + hairW * 0.12, joints.head[1] + hairDrop * 0.28);
      ctx.moveTo(joints.head[0] + hairW * 0.16, joints.head[1] - headRadius * 0.02);
      ctx.quadraticCurveTo(joints.head[0] + hairW * 0.34, joints.head[1] + hairDrop * 0.24, joints.head[0] + hairW * 0.24 + sway * 0.2, joints.head[1] + hairDrop * 0.52);
      ctx.stroke();
      ctx.restore();
    };
    ...
    ctx.fillStyle = `hsl(${(cfg.outfitHue + 14).toFixed(1)} 78% ${(63 + drive * 10).toFixed(1)}%)`;
    ctx.beginPath();
    ctx.arc(joints.head[0], joints.head[1], headRadius, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "hsla(0 0% 100% / 0.16)";
    ctx.beginPath();
    ctx.ellipse(joints.head[0] + headRadius * 0.2, joints.head[1] - headRadius * 0.24, headRadius * 0.2, headRadius * 0.12, -0.35, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "hsla(0 0% 100% / 0.22)";
    ctx.beginPath();
    ctx.arc(joints.head[0] - headRadius * 0.18, joints.head[1] - headRadius * 0.02, Math.max(1.2, headRadius * 0.06), 0, Math.PI * 2);
    ctx.arc(joints.head[0] + headRadius * 0.18, joints.head[1] - headRadius * 0.02, Math.max(1.2, headRadius * 0.06), 0, Math.PI * 2);
    ctx.fill();
    drawSparkles();
  }
