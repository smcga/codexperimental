...
export function normalizeNeonAlleyParams(params: Record<string, number>): NeonAlleyParams {
  ...
  const exposure = clamp(toFiniteNumber(params.exposure, NEON_ALLEY_DEFAULTS.exposure), 0.6, 2.0);
  const hueShift = clamp(toFiniteNumber(params.hueShift, NEON_ALLEY_DEFAULTS.hueShift), 0, 1);
...
export class NeonAlleyEffect implements Effect {
  ...
  constructor() {
    this.webgl = new WebGLEffectBase(NEON_ALLEY_FRAGMENT_SHADER, "neon_alley");
  }
  ...
  render(context: EffectRenderContext): void {
    ...
    if (!this.warned) {
      ...
      console.warn(`[neon_alley] ${reason}. Falling back to neon shapes effect.`);
      this.warned = true;
    }
