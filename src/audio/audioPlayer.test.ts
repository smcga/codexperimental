...
describe("AudioPlayer (Web Audio clock)", () => {
  ...
  it("play() schedules source from pausedAt using context clock", async () => {
    ...
    expect(sources[0].buffer).toBe(fakeBuffer);
    expect(sources[0].start).toHaveBeenCalledWith(5.05, 2.5);
    expect(sources[0].connect).toHaveBeenCalledWith(fakeGain);
    expect(fakeGain.connect).toHaveBeenCalledWith(fakeAnalyser);
    expect(fakeAnalyser.connect).toHaveBeenCalledWith(mockContext.destination);
  });
  ...
  it("falls back to callback decode when single-arg decode throws", async () => {
    mockContext.decodeAudioData = vi.fn(
      (encoded: ArrayBuffer, onSuccess?: (value: AudioBuffer) => void, _onError?: (reason: unknown) => void) => {
        if (!onSuccess) {
          throw new TypeError("Legacy Safari decode signature");
        }
        onSuccess?.(fakeBuffer);
        return undefined;
      }
